...
**Problem**: Cannot connect to ChromaDB or getting connection errors.
...
4. **Try restarting ChromaDB**:
   ```bash
   docker-compose restart chromadb
   ```

5. **Use memory vector store as fallback**:
   ```env
   VECTOR_STORE_TYPE=memory
   ```
...
**Problem**: API authentication errors or rate limit errors.
...
2. **Check API key format**: Should start with `sk-` for OpenAI API keys.

3. **Rate limit errors**: The system includes automatic retry with exponential backoff. If you're hitting rate limits frequently:
...
**Problem**: Vector stores fail to initialize or documents aren't indexed.
...
2. **Check document files exist**:
   ...
   ls -la data/hr_docs/
   ls -la data/it_docs/
   ls -la data/finance_docs/
   ls -la data/legal_docs/
   ```

3. **Verify documents are in correct format**: Files should be `.txt` or `.md` format.

4. **Check minimum document requirements**: Each domain should have at least 20 chunks (configurable via `MIN_CHUNKS`).
...
5. **Clear and rebuild**:
   ...
   docker volume rm company-support-chatbot_chroma-data
...
**Problem**: Configuration not working or defaults not being used.
...
2. **Check for typos**: Variable names are case-sensitive. Common ones:
   - `OPENAI_API_KEY` (not `OPENAI_KEY`)
   - `VECTOR_STORE_TYPE` (not `VECTOR_STORE`)
   - `CHROMA_HOST` (not `CHROMAHOST`)

3. **Restart the application**: Environment variables are loaded at startup.

4. **Check `.env.example`**: Compare your `.env` with `.env.example` for correct variable names.

5. **Validate with typecheck**:
   ```bash
   npm run typecheck
   ```
   Configuration errors will show up during type checking.

### Document Loading Issues

**Problem**: Documents not loading or empty results.
...
3. **Check file size**: Very large files may cause memory issues. Consider splitting them.
...
**Problem**: Application is slow or running out of memory.
...
4. **Use lazy agent loading**: Agents initialize on-demand, reducing startup memory usage.

5. **Reduce TOP_K for retrieval**:
   ```env
   TOP_K=3
   ```
...
**Problem**: Langfuse tracing not working or connection errors.

**Solutions**:
1. **Langfuse is optional**: The system works without Langfuse. If you don't need tracing, disable it:
   ```env
   # Disable Langfuse entirely
   LANGFUSE_ENABLED=false
   ```
   Or simply omit the Langfuse variables:
   ```env
   # Comment out or remove these if not using Langfuse
   # LANGFUSE_ENABLED=true
   # LANGFUSE_PUBLIC_KEY=...
   # LANGFUSE_SECRET_KEY=...
   # LANGFUSE_HOST=...
...
2. **Verify credentials**: Check your Langfuse dashboard for correct keys.

3. **Check host URL**: Default is `https://cloud.langfuse.com`. For self-hosted instances, update `LANGFUSE_HOST`.

4. **Network connectivity**: Ensure your network can reach Langfuse servers.

### Intent Classification Issues

**Problem**: Queries being routed to wrong agents or low confidence scores.

**Solutions**:
1. **Improve document quality**: Ensure domain-specific documents are comprehensive and relevant.

2. **Check classification logs**: Enable debug logging to see classification decisions:
   ```env
   LOG_LEVEL=debug
   ```

3. **Review test queries**: Run the test suite to check classification accuracy:
   ```bash
   npm run dev:test
   ```

4. **Adjust confidence threshold**: If needed, modify the orchestrator's confidence threshold (see source code).
...
2. **Check application logs**: Look for error messages and stack traces.
...
- Check the [ARCHITECTURE.md](./docs/ARCHITECTURE.md) for detailed system documentation
- Review error messages carefully - they often contain specific guidance
- Check GitHub issues for similar problems
- Ensure you're using Node.js 22+ and all dependencies are installed (`npm install`)
